...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(newDN make_features all_testfeatures make_testfeatures print_features
...
#		pssm, atch, seq
#	arch : desired architecture of DN		
#	tag : trial description that will be printed in dnss files
# Out:  predir : directory of predicted proteins from test list
#	dnssdir : directory of dnss prediction files of ^
################################################################
...
sub train_DN {
	...
	`python2 ./train-DN_arch.py $npyfile $modelfile $arch 1>&2`;
...
sub test_DN {
	...
	foreach my $prot (@list){
		...
		my $output = `python2 ./test-DN.py $npy $model $predfile $probfile $targ`;
	}
...
sub predict_SS {
	my ($model, $featfile, $outdir, $device, $tag) = @_;
	...
	`python2 $GLOBAL_PATH/scripts/feat-2-npy.py $featfile $npy`;
	`python2 $GLOBAL_PATH/scripts/test-DN.py $npy $model $predfile $probfile $device 3`;
...
sub all_dnss_files {
	...
	foreach my $prot (@list){
		...
		my $vdnssfile = $outdir . "$prot.vdnss";
		...
		my $equal = make_dnss_file(\@predfiles, $pssmfile, $outfile, $vdnssfile, $header, $ssafile);
		unless ($equal){
